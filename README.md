# Studio Dentistico Dr. Stefano Borioli

Sito web ufficiale dello Studio Dentistico Dr. Stefano Borioli a Intra (VB).

🌐 **Live:** [boriolifederico-source.github.io/studioborioli](https://boriolifederico-source.github.io/studioborioli)

## Info Studio

- 📍 Via S. Vittore, 11, 28921 Intra VB
- 📞 +39 0323 52102
- ✉️ studio.borioli@libero.it
- ⭐ 5.0/5.0 su Google (36+ recensioni)

## Orari

| Giorno | Orario |
|--------|--------|
| Lunedì | 9:00 – 19:00 |
| Martedì | 12:00 – 19:00 |
| Mercoledì | 12:00 – 19:00 |
| Giovedì | 9:00 – 16:00 |
| Venerdì | 9:00 – 16:00 |
| Sabato | 9:00 – 12:00 |
| Domenica | Chiuso |

## Sviluppo

Sito statico in HTML/CSS/JS, deployato via GitHub Pages.

Per modificare: modifica `index.html`, fai commit e push su `main`. GitHub Pages ricostruisce automaticamente in 1-2 minuti.

Vedi [CLAUDE.md](CLAUDE.md) per documentazione completa del progetto.
